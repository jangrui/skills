#!/usr/bin/env bash
# ============================================================================
# sync-opc-skills.sh — 同步 ReScienceLab/opc-skills 上游 skill 到 vendor 目录
#
# 上游特点：
#   - 单仓库多 skill，skill 在 skills/<name>/ 子目录（sparse-checkout 合法：
#     cone 模式认目录，skills/ 是目录）
#   - 面向 solopreneur 的营销/调研/设计技能：Gemini 生图（nanobanana）、
#     logo/banner 制作、Reddit/Twitter/ProductHunt 抓取、需求调研
#     （requesthunt CLI，已发布 GitHub Releases）、SEO/GEO、域名选购、
#     会话知识归档（archive）
#   - Python 脚本依赖 stdlib + 已发布 pip 包（google-genai、pillow），
#     无 workspace 兄弟包（credential/twitter_api 等裸名 import 均为
#     skill 内 scripts/ 同级模块，已验证）
#
# 机制：
...
#
# 用法：
#   ./scripts/sync-opc-skills.sh                     # 检查并同步全部
#   ./scripts/sync-opc-skills.sh reddit               # 只同步一个 skill
#   ./scripts/sync-opc-skills.sh --check              # 仅检查不修改（dry-run）
#
# 同步后请人工 review：
#   git diff -- skills/opc/<name>/ .claude-plugin/marketplace.json
#   git add skills/opc/
...
if [ -n "$TARGET" ]; then
  ...
  if [ "$local_count" = "0" ]; then
    echo "  （本地尚无 opc skill，将全部从上游首次 vendor）"
  fi
...
echo "=== 完成 ==="
if [ "$DRY_RUN" != "1" ]; then
  echo "下一步：git diff -- skills/opc/ .claude-plugin/marketplace.json  人工 review 后 commit"
fi
