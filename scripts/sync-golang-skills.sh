#!/usr/bin/env bash
# ============================================================================
# sync-golang-skills.sh — 同步 samber/cc-skills-golang 的上游 skill 到 vendor 目录
#
# 上游特点（与 lark 同属「单仓库多 skill / 扁平」型）：
#   - 单仓库多 skill：samber/cc-skills-golang 的 skills/ 下有 46 个 golang-* 子目录
#   - 一次 sparse-checkout skills/ 即可拿到全部，无需循环 clone
#   - 所有 skill 共享同一上游 commit（同一仓库的同一 HEAD）
#
# 排除项（与 lark 的差异）：
#   - 排除 evals/：每个 skill 下的 evals/ 是上游 CI 评估数据（evals.json），
#     0 个 SKILL.md 引用它，纯非运行时文件。排除可省 32% 体积。
#   - 保留 references/ 和 assets/：被大量 SKILL.md 引用（43/46 和 6/46），是 skill 知识库。
#
# 机制：
#   - git sparse-checkout 只拉上游 skills/ 子目录（不要 .git/.github/CLAUDE.md 等）
#   - 每个 skill 目录单独保存 .upstream-commit（虽然值相同，但便于单 skill 回退定位）
#   - 同步后自动跑「自包含性自检」，防止上游引入兄弟包依赖导致 vendor 断裂
#
# 用法：
#   ./scripts/sync-golang-skills.sh                  # 检查并同步全部
#   ./scripts/sync-golang-skills.sh golang-benchmark # 只同步 golang-benchmark
...
discover_skills() {
  ...
  find "$SKILLS_DIR" -maxdepth 1 -mindepth 1 -type d -name 'golang-*' 2>/dev/null \
    | sed 's|.*/||' | grep -v '^\.claude-plugin$' | sort
}

# ---------- 参数解析 ----------
# 支持任意位置的 --check/-n（避免 `script.sh golang-benchmark --check` 被当成真实同步）
DRY_RUN=0
TARGET=""
for arg in "$@"; do
  case "$arg" in
...
sync_one() {
  ...
  mkdir -p "$vendor_dir"
  # 排除 evals/（上游 CI 评估数据，0 运行时引用）
  rsync -a ${old_commit:+--delete} --exclude='.upstream-commit' --exclude='evals/' "$src/" "$vendor_dir/"
...
if [ -n "$TARGET" ]; then
  if [ ! -d "$SKILLS_DIR/$TARGET" ]; then
    echo "❌ 本地无 ${TARGET}（本地目录名即 skill key，如 golang-benchmark）"
