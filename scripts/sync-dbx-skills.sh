#!/usr/bin/env bash
# ============================================================================
# sync-dbx-skills.sh — 同步 t8y2/dbx 上游 skill 到 vendor 目录
#
# 上游特点：
#   - 单仓库多 skill 形态（当前仅 skills/dbx/ 一个 skill）
#   - SKILL.md 在 skills/<name>/ 子目录，可用 sparse-checkout
#   - skill 为纯 md 使用指南，运行时依赖独立发布的 npm 包 @dbx-app/cli
#
# 机制：
...
#
# 用法：
#   ./scripts/sync-dbx-skills.sh              # 检查并同步全部
#   ./scripts/sync-dbx-skills.sh dbx          # 只同步 dbx
...
sync_one() {
  ...
  if [ -n "$old_commit" ]; then
    ...
    rsync -a --exclude='.upstream-commit' "$src/" "$vendor_dir/"
  fi
...
if [ -n "$TARGET" ]; then
  if [ ! -d "$SKILLS_DIR/$TARGET" ]; then
    echo "❌ 本地无 ${TARGET}（本地目录名即 skill key，如 dbx）"
  ...
  while IFS= read -r sub; do
    ...
    if [ ! -d "$SKILLS_DIR/$sub" ]; then
      ...
      if [ "$DRY_RUN" = "1" ]; then
        ...
        rsync -a --exclude='.upstream-commit' \
