...
#
# 同步后请人工 review：
#   git diff -- skills/grafana/<category>/<skill>/ .claude-plugin/marketplace.json
...
echo "=== 完成 ==="
if [ "$DRY_RUN" != "1" ]; then
  echo "下一步：git diff -- skills/grafana/ .claude-plugin/marketplace.json  人工 review 后 commit"
fi
