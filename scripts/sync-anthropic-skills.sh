#!/usr/bin/env bash
# ============================================================================
# sync-anthropic-skills.sh — 同步 anthropics/skills 的上游 skill 到 vendor 目录
#
# 上游特点（与 golang/lark 同属「单仓库多 skill / skills/ 子目录」型）：
#   - anthropics/skills 官方仓库，skills/ 下 17 个 skill 子目录（无统一前缀）
#   - 一次 sparse-checkout skills/ 即可拿到全部
#   - 所有 skill 共享同一上游 commit
#
# 与仓库通用约定的两处刻意偏离（勿"修复"）：
#   - 保留各 skill 目录内的 LICENSE.txt：SKILL.md frontmatter 显式引用
#     `license: Complete terms in LICENSE.txt`，且 docx/pdf/pptx/xlsx 四个 skill
#     为 Anthropic 专属条款（非 Apache-2.0），剔除 LICENSE 会丢失法律条款。
#     其余上游的 LICENSE 都在仓库根目录，故通用约定排除之；这里在 skill 本体内。
#   - 保留 skill-creator 的 agents/：grader.md/comparator.md/analyzer.md 被
#     SKILL.md 直接引用，是 skill 本体而非仓库级 Claude 配置。
#
# 机制：
#   - git sparse-checkout 只拉上游 skills/ 子目录（不要 spec/ template/ .claude-plugin/ 等）
#   - 每个 skill 目录单独保存 .upstream-commit
#   - 同步后自动跑「自包含性自检」，防止上游引入兄弟包依赖导致 vendor 断裂
#
# 用法：
#   ./scripts/sync-anthropic-skills.sh              # 检查并同步全部
#   ./scripts/sync-anthropic-skills.sh mcp-builder   # 只同步 mcp-builder
#   ./scripts/sync-anthropic-skills.sh --check       # 仅检查不修改（dry-run）
#
# 同步后请人工 review：
#   git diff -- skills/anthropic/<name>/ .claude-plugin/marketplace.json
...
# 该上游 skill 无统一前缀，靠 SKILL.md 存在性识别；.claude-plugin/ 非 skill 跳过
discover_skills() {
  local d
...
# ---------- 参数解析 ----------
# 支持任意位置的 --check/-n（避免 `script.sh mcp-builder --check` 被当成真实同步）
DRY_RUN=0
TARGET=""
for arg in "$@"; do
  case "$arg" in
...
check_self_contained() {
  ...
  # 检查：可执行脚本里是否 import @xxx/yyy 形式的兄弟包，或裸名引用 workspace 包
  local brother
  ...
  # 该上游 Python 脚本依赖已发布的 pip 包（pillow/anthropic/mcp/playwright/pypdf 等），
  # 属仓库入库决策的 ✅ 项，不算兄弟包依赖，不在此告警。
  if [ "$warnings" = "0" ]; then
    echo "  ✅ 自检通过：无兄弟包依赖"
  else
...
sync_one() {
  ...
  mkdir -p "$vendor_dir"
  # 注意：不排除 LICENSE.txt（见文件头说明）；无 evals/ 类非运行时目录
  rsync -a ${old_commit:+--delete} --exclude='.upstream-commit' "$src/" "$vendor_dir/"
...
if [ -n "$TARGET" ]; then
  if [ ! -d "$SKILLS_DIR/$TARGET" ]; then
    echo "❌ 本地无 ${TARGET}（本地目录名即 skill key，如 mcp-builder）"
  ...
  echo "--- 检测上游新增 skill ---"
  new_added=0
  # 注意：必须用进程替换 < <(...) 而非管道 | while，否则在 set -u 下子 shell 边界会报变量未绑定
  ...
  [ "$new_added" = "0" ] && echo "  （无新增）"
fi

echo ""
echo "=== 完成 ==="
if [ "$DRY_RUN" != "1" ]; then
  echo "下一步：git diff -- skills/anthropic/ .claude-plugin/marketplace.json  人工 review 后 commit"
fi
