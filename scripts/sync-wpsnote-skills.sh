#!/usr/bin/env bash
# ============================================================================
# sync-wpsnote-skills.sh — 同步 wpsnote/wpsnote-skills 上游 skill 到 vendor 目录 skills/wpsnote/
#
# 上游特点（单仓库多 skill / 扁平）：
...
#
# 同步后请人工 review：
#   git diff -- skills/wpsnote/<name>/ .claude-plugin/marketplace.json
#   git add skills/wpsnote/
...
PLUGIN_DIR="$REPO_ROOT/skills/wpsnote"
  ...
  while IFS= read -r sub; do
    ...
    if [ ! -d "$SKILLS_DIR/$sub" ] || [ ! -f "$SKILLS_DIR/$sub/SKILL.md" ]; then
      ...
      if [ "$DRY_RUN" = "1" ]; then
        ...
        echo "  ⚠️  请手动编辑 marketplace.json 中 wpsnote-skills 的 skills 数组加入 \"$sub\""
        UPDATED_SKILLS+=("$sub")
        local_added=1
      fi
    fi
...
echo "=== 完成 ==="
if [ "$DRY_RUN" != "1" ]; then
  echo "下一步：git diff -- skills/wpsnote/ .claude-plugin/marketplace.json  人工 review 后 commit"
fi
